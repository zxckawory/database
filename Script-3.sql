select ("FirstName"|| ' ' ||"LastName") as "RunnerName", date_part('year', age(current_date, "DateOfBirth")) as "Age", "Gender", row_number() over (partition by e."EventId"order by "RaceTime") AS "Position", (("RaceTime" / 60 / 60)::text || 'h ' || (("RaceTime" / 60 % 60)::text || 'min ' ||  ("RaceTime" % 60)::text || 'sec')) as "RaceTime", "MarathonName", "EventName"  from "Marathon" m 
